...
export default function ComplaintModal({ isOpen, onClose, complaint, user }: ComplaintModalProps) {
        ...
        <Dialog open={isOpen} onClose={onClose} className="relative z-50">
            ...
            <div className="fixed inset-0 flex items-center justify-center p-4">
                <Dialog.Panel className="w-full max-w-2xl bg-white rounded-xl shadow-lg p-6 space-y-4 relative">
                    ...
                    <div className="pt-4 border-t border-gray-200">
                        ...
                        <div className="flex justify-center gap-4">
                            <button
                                onClick={() => handleFeedbackSubmit(complaint.id, 'satisfied')}
                                className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium shadow-sm  disabled:text-black
                                    ${satisfaction === 'yes' ? 'bg-green-800 text-white disabled:bg-green-200 disabled:hover:bg-green-200' : 'bg-green-500 text-white hover:bg-green-600 disabled:bg-green-300 disabled:hover:bg-green-300 '}
                  ${previousSatisfaction === 'yes' && satisfaction === 'no' ? 'ring-2 ring-yellow-400' : ''}
                ...
                `}
                            ...
                            <button
                                onClick={() => handleFeedbackSubmit(complaint.id, 'unsatisfied')}
                                className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium shadow-sm   disabled:text-black
                  ${satisfaction === 'no' ? 'bg-red-800 text-white disabled:bg-red-200 disabled:hover:bg-red-200' : 'bg-red-500 text-white hover:bg-red-600 disabled:bg-red-300 disabled:hover:bg-red-300 '}
                  ${previousSatisfaction === 'no' && satisfaction === 'yes' ? 'ring-2 ring-yellow-400' : ''}
                   hover:text-white
                `}
                                disabled={isSubmitting || satisfaction !== null} // Disable button if already submitted
                            >
